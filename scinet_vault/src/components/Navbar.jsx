          ...
          <div className="hidden md:flex items-center space-x-6">
            {isConnected && navigationItems.map((item) => (
              <motion.div key={item.name} whileHover={{ y: -2 }}>
                <Link
                  to={item.path}
                  className="flex items-center space-x-2 text-gray-300 hover:text-primary-400 transition-all duration-300 px-3 py-2 rounded-xl hover:bg-white/10 group"
                >
              ...
              {isConnected && navigationItems.map((item) => (
                <motion.div key={item.name} whileHover={{ x: 5 }}>
                  <Link
                    ...
                    className="flex items-center space-x-3 text-gray-300 hover:text-primary-400 transition-all duration-300 py-3 px-4 rounded-xl hover:bg-white/10 group"
                  >
                    <item.icon className="h-4 w-4 group-hover:scale-110 transition-transform duration-200" />
                    <span className="font-medium">{item.name}</span>
                  </Link>
